...
## Backup Library If Exists ###
	if [ -f $BlingLib ]; then
	   ...
	   ##  cp -v $BlingLib ./$BackupDir/"$BlingBackup"_"$(date +%Y%m%d%H%M)".blib
	   cp -v $BlingLib ./$BackupDir/"$BlingBackup"_"$(date "$DateFormat")".blib
...
# for i in $FuncDir/*.func;do cat $i > $BlingLib;done
	...
	## cat $FuncDir/*.func > $LibDir/BlingBuild_"$(date +%Y%m%d%H%M)".blib
	 cat $FuncDir/*.func > $LibDir/BlingBuild_"$(date "$DateFormat")".blib
